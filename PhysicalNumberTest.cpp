...
int main() {
  ...
  if (signal == 0) {
    ...
    .CHECK_EQUAL(m2+m1 < m3, false)
    .CHECK_EQUAL(m2-m1 < m3, true)
    .CHECK_EQUAL(m2+m2 == m3, false)
    .CHECK_EQUAL(m3+m3 > m3, true)
    .CHECK_EQUAL(t2+t3 <= t1, true)
    .CHECK_EQUAL(t2+t3 >= t3+t2, true)
    .CHECK_OUTPUT(-l1, "-1[km]")
    .CHECK_OUTPUT(-l2, "-1[m]")
    .CHECK_OUTPUT(-l3, "-100[cm]")
    .CHECK_OUTPUT(+l1, "1[km]")
    .CHECK_OUTPUT(+l2, "1[m]")
    .CHECK_OUTPUT(+l3, "100[cm]")
    ...
    .CHECK_OUTPUT(+m1, "1[ton]")
    .CHECK_OUTPUT(+m3, "1000[g]")
    .CHECK_OUTPUT(l2+l3, "2[m]")
    .CHECK_OUTPUT(l2-l3, "0[m]")
    ...
    .CHECK_OUTPUT(t2+t3+t2, "3[min]")
    .CHECK_OUTPUT(t3-t2+t2, "60[sec]")
    .CHECK_OUTPUT(m3-m2-m3, "-1000[g]")
    .CHECK_OUTPUT(m2+m3-m2, "1[kg]")
    .CHECK_OK(t1+=t2)
    .CHECK_OK(t2++)
    .CHECK_OK(m1+=m3)
    .CHECK_OK(m2--)
